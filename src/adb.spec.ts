...
test("execOut() should resolve", async t => {
  const [[adb]] = fake()(["ok", "", 0]);
  const stream = td.instance(WriteStream);
  t.falsy(await adb.execOut(stream, "echo", "ok"));
  td.verify(stream.close());
});
test("execOut() should reject on error", async t => {
  const [[adb]] = fake()(["problem", "", 1]);
  const stream = td.instance(WriteStream);
  await t.throwsAsync(adb.execOut(stream, "echo", "ok"), {
    message: '{"error":{"code":1}}'
  });
});

test("createBackupTar()", async t => {
  ...
  t.deepEqual(await adb.listUbuntuBackups("./src/__test-helpers/"), [
    {
      ...
      dir: "src/__test-helpers/fake_backup",
      serialno: 1337,
      size: 42,
      time: "2000"
    }
...
test("createUbuntuTouchBackup()", async t => {
  ...
  const progress = td.func(p => {});
  t.like(await adb.createUbuntuTouchBackup(dest, undefined, "data", progress), {
    codename: "123",
    restorations: [],
    serialno: "123",
    size: 246
  ...
  td.verify(progress(0));
  await t.throwsAsync(adb_error.createUbuntuTouchBackup(dest));
});
